...
for (var i = 0; i < themes.length; i++) {
  ...
  document.getElementById(themeName).addEventListener("click", function () {
    ...
    const scroller = document.querySelectorAll("." + mainTheme + "-scroller");
    ...
    for (var v = 0; v < itemstext2.length; v++) {
      ...
      for (var v = 0; v < scroller.length; v++) {
        if (!scroller[v].classList.contains('themeContent')) {
          scroller[v].classList.remove(mainTheme + "-scroller");
          scroller[v].classList.add(themeName + "-scroller");
        }
      }
    mainTheme = themeName;
  })
...
// surah list for left navbar start 

const leftSurahList = document.querySelector("#surah-list");

let leftSurahJsonArray = "";

fetch("./api/surah.json")
  .then((response) => response.json())
  .then((data) => {
    leftSurahJsonArray = data;
    leftSurahFunc();
  });

function leftSurahFunc() {
  var leftSurahItem = "";

  for (var i = 0; i < leftSurahJsonArray.length; i++) {
    var surah = leftSurahJsonArray[i];
    var place = surah["place"];
    var type = surah["type"];
    var ayahCount = surah["count"];
    var title = surah["title"];
    ...
    leftSurahItem +
    `
    <div class="surah-wrapper dark-blue-content">
   
    <span class="surah-count dark-blue-bg dark-blue-text">` +
    (i + 1) +
    `</span>

    <div class="surah-flex">
        <div class="titleAndPlace">
...
// surah list for left navbar end 
